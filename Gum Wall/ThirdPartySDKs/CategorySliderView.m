...
- (instancetype)initWithFrame:(CGRect)frame andCategoryViews:(NSArray *)categoryViews sliderDirection:(SliderDirection)direction categorySelectionBlock:(categorySelected)block {
    if ((self = [super initWithFrame:frame])) {
        ...
        [self.scrollView setBackgroundColor:[UIColor colorWithRed:35 green:213.0/255.0 blue:247.0/255.0 alpha:0.0]];
...
- (void)categoryViewTapped:(UITapGestureRecognizer *)tap {
    ...
    for (UIView *categoryView in self.categoryViews) {
        if ([categoryView isKindOfClass:[UILabel class]]) {
            UILabel *categoryLabel = (UILabel *)categoryView;
            if (categoryLabel.tag == tap.view.tag) {
                [categoryLabel setTextColor:[UIColor colorWithRed:42.0/255.0 green:213.0/255.0 blue:247.0/255.0 alpha:1.0]];
            }
            else {
...
- (void)markSelectionOnCategoryNumber:(int)categoyNumber {
    for (UIView *categoryView in self.categoryViews) {
        if ([categoryView isKindOfClass:[UILabel class]]) {
            UILabel *categoryLabel = (UILabel *)categoryView;
            if (categoryLabel.tag == categoyNumber) {
                [categoryLabel setTextColor:[UIColor colorWithRed:42.0/255.0 green:213.0/255.0 blue:247.0/255.0 alpha:1.0]];
                [self slideItemAtIndex:categoyNumber animated:YES];
            }
            else {
                [categoryLabel setTextColor:[UIColor whiteColor]];
            }
